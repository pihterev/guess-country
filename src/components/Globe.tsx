...
import getMapData from '../Map/MapDataProvider';
...
export class Globe extends Component<GlobeProps> {
  onCountryClick (e) {
      const result = this.props.handlerAnswer(e.point.options.value);

      if (result) {
        data[e.point.index]['color'] = 'blue';
        e.point.series.setData(data);
      }
  };

  componentDidMount() {
    const topology = getMapData();

    const chart = Highcharts.mapChart('container-globe', {
      chart: {
        map: topology,
      },
